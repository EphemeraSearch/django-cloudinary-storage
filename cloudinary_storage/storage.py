...
class StaticCloudinaryStorage(MediaCloudinaryStorage):
    ...
    file_hash = (
        ...
        if PY3
        else get_method_function(HashedFilesMixin.file_hash)
    )
    clean_name = (
        HashedFilesMixin.clean_name
        if PY3
        else get_method_function(HashedFilesMixin.clean_name)
    )
...
class HashCloudinaryMixin(object):
    ...
    stored_name = (
        ...
        if PY3
        else get_method_function(HashedFilesMixin.stored_name)
    )
